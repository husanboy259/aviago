-- ================================================================
-- Airo Go — Supabase Schema
-- Single postgres database, safe to re-run (IF NOT EXISTS)
...
-- ════════════════════════════════════════
-- USERS  (shared by auth-service & user-service)
-- ════════════════════════════════════════
...
-- Add password column if upgrading from old schema
...
CREATE TABLE IF NOT EXISTS orders (
  ...
  restaurant_name       VARCHAR(100)  NOT NULL DEFAULT '',
  created_at            TIMESTAMPTZ   NOT NULL DEFAULT NOW(),
  updated_at            TIMESTAMPTZ   NOT NULL DEFAULT NOW()
);
...
CREATE TABLE IF NOT EXISTS drones (
  ...
  speed             DECIMAL(6,2),
  heading           DECIMAL(6,2),
  max_payload_grams INTEGER       NOT NULL DEFAULT 1000,
  max_range_km      DECIMAL(6,2)  NOT NULL DEFAULT 10,
